...
class DarpaActions extends Actions {
  ...
  def expand(mention: Mention): Mention = new TextBoundMention(mention.labels,
    subgraph(mention.tokenInterval, mention.document.sentences(mention.sentence).dependencies.get),
    mention.sentence,
    mention.document,
    mention.keep,
    mention.foundBy)
  ...
  def mkNERMentions(mentions: Seq[Mention], state: State): Seq[Mention] = {
    mentions flatMap { m =>
      val candidates = state.mentionsFor(m.sentence, m.tokenInterval.toSeq).map(mn =>
      if (mn.labels.contains("Unresolved")) expand(mn) else mn)
